...
class TweetQuery(graphene.ObjectType):
    tweets = DjangoFilterConnectionField(TweetNode, comment_to_pk=graphene.Int(
    ), username=graphene.String(), exclude_comment=graphene.Boolean(), timeline=graphene.Boolean())
...
class PostTweet(graphene.Mutation):
    ...
    def mutate(root, info, text, file: InMemoryUploadedFile = None, comment_to=None):
        ...
        new_tweet = Tweet(text=text, user=info.context.user,
                          image=file, comment_to=tweet_to_comment)
        new_tweet.full_clean()
        new_tweet.save()
        if tweet_to_comment and tweet_to_comment.user != info.context.user:
            user: User = tweet_to_comment.user
            user.email_user(f"New reply from {info.context.user.username}",
                            f"Click link to read more. https://arsaiz.xyz/tweets/{tweet_to_comment.id}")
...
class RetweetMutation(graphene.Mutation):
    ...
    def mutate(root, info, tweet_id):
        ...
        if not tweet_query.first():
            return None
        retweeted = tweet_query.filter(
            retweets__id=info.context.user.id).first()
        tweet = tweet_query.first()
        if retweeted:
