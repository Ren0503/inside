...
class AcceptFollow(graphene.Mutation):
    ...
    def mutate(cls, info, user_id):
        ...
        is_requested = info.context.user.follow_requests.filter(
            id=user_id).exists()
        if is_requested:
...
class FollowUser(graphene.Mutation):
    ...
    def mutate(cls, info, user_id):
        ...
        if not user_query.first():
            return None
        followed = user_query.filter(
            followers__id=info.context.user.id).exists()
        user: User = user_query.first()
        if followed:
            user.followers.remove(info.context.user)
        else:
            if user.private:
                requested = user_query.filter(
                    follow_requests__id=info.context.user.id)
                if requested:
